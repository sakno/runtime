...
namespace System.Numerics
{
    internal static partial class BigIntegerCalculator
    {
        ...
        public static int Gcd(ReadOnlySpan<uint> left, ReadOnlySpan<uint> right, Span<uint> result)
        {
            ...
            var count = Gcd(result, rightCopy);
        ...
        private static int Gcd(Span<uint> left, Span<uint> right)
        {
            Debug.Assert(left.Length >= 2);
            Debug.Assert(right.Length >= 2);
        ...
        private static void Reduce(ref Span<uint> bits, ReadOnlySpan<uint> modulus)
        {
            ...
            if (bits.Length >= modulus.Length)
            {
                ...
                bits = bits.Slice(0, ActualLength(bits.Slice(0, modulus.Length)));
            }
        }
